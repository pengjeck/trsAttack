#include "sms4.h"

SMS4::SMS4(Parameter *parameter)
{
    this->IsValue=parameter->IsValueBuild;
    this->SBoxLength=parameter->SBoxLength;
    this->HM00=parameter->HM00;
    this->HM01=parameter->HM01;
    this->Model=parameter->Model;
}
/*参数说明：
 * IsValue：是否根据值建立模板
 * SBoxLength：2的子密钥比特数次幂
 * HM00：泄漏点1
 * HM01：泄漏点2
 * Model：功耗模型
 * */
const unsigned int SMS4::HM[256] =
{
    0,1,1,2,1,2,2,3,1,2,2,3,2,3,3,4,1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,
    5,1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,2,3,3,4,3,4,4,5,3,4,4,5,4,5,
    5,6,1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,2,3,3,4,3,4,4,5,3,4,4,5,4,
    5,5,6,2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,3,4,4,5,4,5,5,6,4,5,5,6,
    5,6,6,7,1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,2,3,3,4,3,4,4,5,3,4,4,
    5,4,5,5,6,2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,3,4,4,5,4,5,5,6,4,5,
    5,6,5,6,6,7,2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,3,4,4,5,4,5,5,6,4,
    5,5,6,5,6,6,7,3,4,4,5,4,5,5,6,4,5,5,6,5,6,6,7,4,5,5,6,5,6,6,7,
    5,6,6,7,6,7,7,8
};

const QString SMS4::SMS4_Sbox[256] =
{
     "d6","90","e9","fe","cc","e1","3d","b7","16","b6","14","c2","28","fb","2c","05",
     "2b","67","9a","76","2a","be","04","c3","aa","44","13","26","49","86","06","99",
     "9c","42","50","f4","91","ef","98","7a","33","54","0b","43","ed","cf","ac","62",
     "e4","b3","1c","a9","c9","08","e8","95","80","df","94","fa","75","8f","3f","a6",
     "47","07","a7","fc","f3","73","17","ba","83","59","3c","19","e6","85","4f","a8",
     "68","6b","81","b2","71","64","da","8b","f8","eb","0f","4b","70","56","9d","35",
     "1e","24","0e","5e","63","58","d1","a2","25","22","7c","3b","01","21","78","87",
     "d4","00","46","57","9f","d3","27","52","4c","36","02","e7","a0","c4","c8","9e",
     "ea","bf","8a","d2","40","c7","38","b5","a3","f7","f2","ce","f9","61","15","a1",
     "e0","ae","5d","a4","9b","34","1a","55","ad","93","32","30","f5","8c","b1","e3",
     "1d","f6","e2","2e","82","66","ca","60","c0","29","23","ab","0d","53","4e","6f",
     "d5","db","37","45","de","fd","8e","2f","03","ff","6a","72","6d","6c","5b","51",
     "8d","1b","af","92","bb","dd","bc","7f","11","d9","5c","41","1f","10","5a","d8",
     "0a","c1","31","88","a5","cd","7b","bd","2d","74","d0","12","b8","e5","b4","b0",
     "89","69","97","4a","0c","96","77","7e","65","b9","f1","09","c5","6e","c6","84",
     "18","f0","7d","ec","3a","dc","4d","20","79","ee","5f","3e","d7","cb","39","48"
};


/*
 * 未知密钥下计算中间值
 * 一条明文对应256个中间值
 * */
vector<int> SMS4::NoneProfileMiddle(QString plain_s,int SNum)
{
        int firststate,secondstate;
        bool ok;
        vector<int> middle_subkey;
        middle_subkey.resize(this->SBoxLength,0);
        QString tem1=plain_s.mid(SNum*3,2);
        QString tem2=plain_s.mid((SNum+4)*3,2);
        QString tem3=plain_s.mid((SNum+8)*3,2);
        QString tem4=plain_s.mid((SNum+12)*3,2);
        int plain_int2 = tem2.toInt(&ok,16);
        int plain_int3 = tem3.toInt(&ok,16);
        int plain_int4 = tem4.toInt(&ok,16);
        for(int i=0;i<this->SBoxLength;i++)
        {
            int middle_tmp=plain_int2^plain_int3^plain_int4;
            int pxork=middle_tmp^i;
            int sboxmiddle=SMS4_Sbox[pxork].toInt(&ok,16);
            if(this->HM00==0)
                 firststate=middle_tmp;
            if(this->HM00==1)
                 firststate=pxork;
            if(this->HM00==2)
                 firststate=sboxmiddle;
            if(this->HM01==1)
                 secondstate=pxork;
            if(this->HM01==2)
                 secondstate=sboxmiddle;
            if(this->Model=="HanMing Weight")
            {
                if(this->IsValue)
                    middle_subkey[i]=firststate;
                else
                    middle_subkey[i]=HM[firststate];
            }
            else
            {
            if(this->IsValue)
                middle_subkey[i]=firststate^secondstate;
            else
                middle_subkey[i]=HM[firststate^secondstate];
            }
         }

    return middle_subkey;
}

/*
 * 已知密钥下计算中间值
 * 一条明文对应一个中间值
 * */
int SMS4::ProfileMiddle(QString plain_s,int correctkey,int SNum)
{
        int firststate,secondstate;
        int middle_subkey;
        bool ok;
        QString tem1=plain_s.mid(SNum*3,2);
        QString tem2=plain_s.mid((SNum+4)*3,2);
        QString tem3=plain_s.mid((SNum+8)*3,2);
        QString tem4=plain_s.mid((SNum+12)*3,2);
        int plain_int1 = tem1.toInt(&ok,16);int plain_int2 = tem2.toInt(&ok,16);
        int plain_int3 = tem3.toInt(&ok,16);int plain_int4 = tem4.toInt(&ok,16);
        int middle_tmp=plain_int2^plain_int3^plain_int4;
            int pxork=middle_tmp^correctkey;
            int sboxmiddle=SMS4_Sbox[pxork].toInt(&ok,16);
            if(this->HM00==0)
                 firststate=middle_tmp;
            if(this->HM00==1)
                firststate=pxork;
            if(this->HM00==2)
                 firststate=sboxmiddle;
            if(this->HM01==1)
                 secondstate=pxork;
            if(this->HM01==2)
                 secondstate=sboxmiddle;
            if(this->Model=="HanMing Weight")
            {
                if(this->IsValue)
                    middle_subkey=firststate;
                else
                    middle_subkey=HM[firststate];
            }
            else
            {
            if(this->IsValue)
                middle_subkey=firststate^secondstate;
            else
                middle_subkey=HM[firststate^secondstate];
            }

    return middle_subkey;
}
